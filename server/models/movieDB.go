...
func (m *DBModel) All(genre ...int) ([]*Movie, error) {
	...
	for rows.Next() {
		...
		queryGenre := `SELECT  mg.id, mg.movie_id, mg.genre_id, g.genre_name
				FROM
					movies_genres mg
					LEFT JOIN genres g on (g.id = mg.genre_id)
				WHERE
					mg.movie_id = $1`
		genreRows, _ := m.DB.QueryContext(ctx, queryGenre, movie.ID)
...
func (m *DBModel) PostMovies(movie Movie) int64 {

	query := `INSERT INTO movies(title,description,year, release_date,runtime, rating, mpaa_rating) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id`
	var id int64
	err := m.DB.QueryRow(query, movie.Title, movie.Description, movie.Year, movie.ReleaseDate, movie.Runtime, movie.Rating, movie.MPAARating).Scan(&id)
	if err != nil {
